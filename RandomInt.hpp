...
public:
RandomInt(int low, int high, unsigned int repeats = 0) : _low(low),
_diff(high-low+1), _maxRepeats(repeats)
{
if (low >= high)
...
if (returnVal == endPoint && _generatedNumbers[returnVal] ==
_maxRepeats)
{
...
RandomInt::_rng{static_cast<uint_fast32_t>(std::chrono::system_clock::now().time_since_epoch().count())};
}
#endif
